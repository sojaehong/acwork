...
export function enrichScheduleData(schedule) {
  ...
  const complexityConfig =
    COMPLEXITY_CONFIG[complexity] || COMPLEXITY_CONFIG.medium
...
export function calculateAdvancedStats(schedules) {
  ...
  enriched.forEach((schedule) => {
    ...
    stats.byComplexity[complexityLabel] =
      (stats.byComplexity[complexityLabel] || 0) + 1
    ...
    if (schedule.tasks && schedule.tasks.length > 0) {
      schedule.tasks.forEach((task) => {
        ...
        stats.byWorkType[taskName] =
          (stats.byWorkType[taskName] || 0) + taskCount
      })
    }

    // 건물별 집계
    if (schedule.building) {
      stats.byBuilding[schedule.building] =
        (stats.byBuilding[schedule.building] || 0) + 1
    }
  })

  stats.averageProgress =
    enriched.length > 0 ? Math.round(totalProgress / enriched.length) : 0
  stats.efficiency =
    (stats.byCategory.completed / Math.max(stats.total, 1)) * 100
...
export const categorizeSchedules = memoize(
  (schedules) => {
    const enriched = schedules.map(enrichScheduleData)
